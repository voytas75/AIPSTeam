...
1. The system tries to retrieve web results using SerpApi. If no data is returned, it catches the error and tries to retrieve web results using EXA. If again no data is returned, it catches the error and tries to retrieve web results using Serper.
2. If no web results are returned from all providers, an error is thrown.
